...
async function toOada(soils) {
  ...
  await Promise.each(Object.keys(soils), async (k) => {
    ...
    // 2. Handle the rest of the soil keys
...
export async function indexSoils(soils) {
...
async function geohashTileMapUnits(mapUnitPolygons) {
  let sevens = {};
  await Promise.each(Object.keys(mapUnitPolygons), async (mupkey) => {
    ...
    await Promise.each(polys, async poly => {
      ...
      keyStrings = poly[0]
        .map(([lng, lat]) => gh.encode(lat, lng, 7))
  ...
  await Promise.each(Object.keys(sevens), async (hash) => {
...
export async function toQuadTiles(soils) {
  console.log(s2);
...
export async function getSeriesDescriptions(soils) {
